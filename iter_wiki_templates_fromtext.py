# -*- coding: utf-8 -*-
"""
Created on Mon Jun 30 16:41:19 2014
...
a module similar to iter_wiki but for finding links in templates, this time by parsing the text of the template, rather than going by SQL, where links have templates inserted
"""
import os
import re 
import MySQLdb
...
def page_parser(elem, namespaces, db):  #namesoaces will have format: {'a':'http://www.mediawiki.org/xml/export-0.8/'}
    ...
    if int(elem.xpath('a:ns', namespaces=namespaces)[0].text) == 10: #if namespace indicates that this is a template
        ...
        try:        
            temp_text = elem.xpath('a:revision/a:text', namespaces=namespaces)[0].text.encode('utf-8')
        except AttributeError:
            print(title_text)
            temp_text=b''
        urlpattern=re.compile(b'(?<=\[)http://.*?[\s\]]|(?<=\[)https://.*?[\s\]]|(?<=\[)ftp://.*?[\s\]]|(?<=\[)//.*?[\s\]]')
        ext_links=urlpattern.findall(temp_text)
        ext_links=[x[:-1] for x in ext_links]
...
def iter_temp_xml(xml_file, parse_func=page_parser, nskey=''):
    ...
    with open('temp_wiki_urls_fromtext.txt','wb') as urltempfile:
        ...
        tempdf.to_csv('temp_wiki_full_fromtext.csv',sep='\t')
    #clear iterator
    del iter_tree
