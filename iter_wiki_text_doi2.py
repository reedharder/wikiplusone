...
def page_parser_plus(elem, namespaces, comment_pattern, inline_pattern, outline_pattern, link_pattern, url_sub, doi_pattern, doi_sub, isbn_pattern, isbn_sub, extlink_pattern, type_pattern,type_sub,pmid_pattern,pmid_sub,pmc_pattern,pmc_sub,arxiv_pattern,arxiv_sub,eprint_pattern,eprint_sub,  archive_pattern,archive_sub,free_cites=True, free_links=True, include_nolinkpages=False ):  #namesoaces will have format: {'a':'http://www.mediawiki.org/xml/export-0.8/'}
    ...
    if int(elem.xpath('a:ns', namespaces=namespaces)[0].text) == 0: #if namespace indicates that this is a template
        ...
        for ref in re_footnote_refs:
            ...
            #Remove spaces from ref for easier parsing
            ref_nospace=ref.replace(b' ', b'')
            
            
            #pull out dois from ref
            try:
                doi=doi_pattern.findall(ref_nospace)[0]
            except IndexError:
                if ref_type == b'doi': #if cite is doi type, grab number from expected second field and clean it
                    doi = ref_nospace.split(b'|')[1].rstrip().lstrip()
            ...
            #pull out dois from ref
            try:
                isbn=isbn_pattern.findall(ref_nospace)[0]
            except IndexError:
                if ref_type == b'isbn': #if cite is isbn type, grab number from expected second field and clean it
                    isbn = ref_nospace.split(b'|')[1].rstrip().lstrip().replace(b'-',b'') 
            ...
            #pull out pmids from ref
            try:
                pmc=pmc_pattern.findall(ref_nospace)[0]
            except IndexError:
                pmc=b''
            else:
                #remove end char
            ...
            try: #get arxiv params from ref
                refarxiv=arxiv_pattern.findall(ref_nospace)[0]
            except IndexError:
                refarxiv=b''
                
            try: #eprint params from list
                refeprint=eprint_pattern.findall(ref_nospace)[0]
            except IndexError:
                refeprint=b''
...
def iter_parse_xml(xml_file, parse_func=page_parser_plus, nskey=''):
    ...
    with open('temp_wiki_urls_plus.txt','wb') as urltempfile:
        ...
        doi_pattern=re.compile(b'\|doi=.*?[|}]',re.DOTALL) #pattern to find dois in citation templates
        doi_sub=re.compile(b'\|doi=') # etc, see comment at top of this block
        pmid_pattern=re.compile(b'\|pmid=.*?[|}]',re.DOTALL)
        pmid_sub=re.compile(b'\|pmid=')
        pmc_pattern=re.compile(b'\|pmc=.*?[|}]',re.DOTALL)
        pmc_sub=re.compile(b'\|pmc=')
        arxiv_pattern=re.compile(b'\|arxiv=.*?[|}]',re.DOTALL)
        arxiv_sub=re.compile(b'\|arxiv=')
        eprint_pattern=re.compile(b'\|eprint=\.*?[|}]',re.DOTALL)
...
def get_goog_ind(bigdf_row, safedomains):
    ...
    if sitename=='google':
        spliturl=urlsplit(bigdf_row['url']) #split first link of row into components
        #get base path
        try:
            basepath=spliturl[2].split("/")[1]
        except IndexError:
            basepath=''
        #if first part of netloc or first part path is a safedomain, keep link        
        if spliturl[1].split(".")[0] in safedomains or basepath in safedomains:
            return None
        else:
...
#keep only reasonable google domains in set of links (no random searches thank you) (will take  a df of domain data as output by domain_parser, or another df containing urls, the sitename of each link and index as a column
...
def google_filter(domdf, custom_filter='default'):
    #set up list of domains to save
    if custom_filter=='default':
        safedomains=["books","about","corporate","events","finance","gwt","help","insights","logos","intl","pacman","patents","press","publicdata","support","trends","webmaster"]
    else:
