...
def iter_wiki_xml(xml_file, parse_func=page_parser, nskey=''):
    ...
    with open('dat_wiki_full.csv', 'w', newline='', encoding='utf-8') as fullfile, open('dat_wiki_domaggr.csv', 'w', newline='', encoding='utf-8') as domfile, open('dat_wiki_googdomsplit.csv', 'w', newline='', encoding='utf-8') as gdomfile, open('dat_wiki_article_id.csv', 'w', newline='', encoding='utf-8') as article_ind, open('dat_wiki_urls.txt','w',encoding='utf-8') as urltxtfile:
        ...
        fg = csv.writer(domfile, delimiter='\t')
        ...
        for event, elem in iter_tree: 
           ...
           if len(parsed_page) > 0:
               if len(parsed_page['ext_links'])>0:
                   ...
                   pattern=re.compile(b'://books.google.com/books\?id')
                   for link in parsed_page['ext_links']:
                        ...
                        if pattern.search(link):
                            ...
                            googbook_id=idpattern.findall(link.split(b'&')[0])
                        ...
                        #get domain
                        dompattern=re.compile(b"http://.*?(?=/)|https://.*?(?=/)")
                        doms=dompattern.findall(link)
                        #GET LINE BELOW INTO LINE ABOVE
                        if len(doms)==1:
                            dom=doms[0]
                            archive=b'_no'
                        elif len(doms)==0:
                            subpattern=re.compile(b"http://.*?(?=\Z)|https://.*?(?=/Z)")
                            dom=subpattern.findall(link)[0]
                            archive=b'_no'
                        elif len(doms)==2:
                            dom=doms[1]
                            archive=dom[0]
                        ...
                        row=[index,parsed_page['title'],parsed_page['wiki_id'],parsed_page['num_links'],link,dom,goog_bookid,archive]
                        ffull.writerow(row)
